...
public class CashOpeningService {
    ...
    private static final Logger LOG = Logger.getLogger(
        CashOpeningService.class.getName()
    );
    ...
    public Respuesta getCashOpening(Long id) {
        try {
            Query qryCashOpening = em.createNamedQuery(
                "CashOpening.findById",
                CashOpening.class
            );
            ...
            return new Respuesta(
                ...
                "CashOpening",
                new CashOpeningDTO(
                    (CashOpening) qryCashOpening.getSingleResult()
                )
            );
    ...
    public Respuesta getCashOpenings() {
        try {
            ...
            List<CashOpening> cashOpenings = (List<
                CashOpening
            >) query.getResultList();
    ...
    public Respuesta guardarCashOpening(CashOpeningDTO cashOpeningDto) {
        try {
            ...
            return new Respuesta(
                ...
                "",
                "CashOpening",
                new CashOpeningDTO(cashOpening)
            );
        } catch (Exception ex) {
            LOG.log(
                Level.SEVERE,
    ...
    public Respuesta closeCashOpening(
        Long id,
        Long closingAmount,
        String notes
    ) {
