...
public class DiningTableService {
    ...
    public Respuesta getDiningTable(Long id) {
        try {
            ...
            return new Respuesta(true, CodigoRespuesta.CORRECTO, "", "", "DiningTable", new DiningTableDTO((DiningTable) qryDiningTable.getSingleResult()));

        } catch (NoResultException ex) {
            return new Respuesta(false, CodigoRespuesta.ERROR_NOENCONTRADO, "No existe una mesa con el código ingresado.", "getDiningTable NoResultException");
        } catch (NonUniqueResultException ex) {
            LOG.log(Level.SEVERE, "Ocurrió un error al consultar la mesa.", ex);
            return new Respuesta(false, CodigoRespuesta.ERROR_INTERNO, "Ocurrió un error al consultar la mesa.", "getDiningTable NonUniqueResultException");
        } catch (Exception ex) {
            LOG.log(Level.SEVERE, "Ocurrió un error al consultar la mesa.", ex);
            return new Respuesta(false, CodigoRespuesta.ERROR_INTERNO, "Ocurrió un error al consultar la mesa.", "getDiningTable " + ex.getMessage());
        }
    }
    ...
    public Respuesta getDiningTables() {
        try {
            ...
            return new Respuesta(false, CodigoRespuesta.ERROR_NOENCONTRADO, "No existen mesas registradas.", "getDiningTables NoResultException");
        } catch (Exception ex) {
            LOG.log(Level.SEVERE, "Ocurrió un error al consultar las mesas.", ex);
            return new Respuesta(false, CodigoRespuesta.ERROR_INTERNO, "Ocurrió un error al consultar las mesas.", "getDiningTables " + ex.getMessage());
        }
    }
    ...
    public Respuesta guardarDiningTable(DiningTableDTO diningTableDto) {
        try {
            ...
            em.flush();
            return new Respuesta(true, CodigoRespuesta.CORRECTO, "", "", "DiningTable", new DiningTableDTO(diningTable));
        } catch (Exception ex) {
            LOG.log(Level.SEVERE, "Ocurrió un error al guardar la mesa.", ex);
            return new Respuesta(false, CodigoRespuesta.ERROR_INTERNO, "Ocurrió un error al guardar la mesa.", "guardarDiningTable " + ex.getMessage());
        }
    }
    ...
    public Respuesta eliminarDiningTable(Long id) {
        try {
            ...
            if (id != null && id > 0) {
                ...
                if (diningTable == null) {
                    return new Respuesta(false, CodigoRespuesta.ERROR_NOENCONTRADO, "No se encontró la mesa a eliminar.", "eliminarDiningTable NoResultException");
                }
                em.remove(diningTable);
            } else {
                return new Respuesta(false, CodigoRespuesta.ERROR_NOENCONTRADO, "Debe cargar la mesa a eliminar.", "eliminarDiningTable NoResultException");
            }
            em.flush();
            return new Respuesta(true, CodigoRespuesta.CORRECTO, "", "");
        } catch (Exception ex) {
            if (ex.getCause() != null && ex.getCause().getCause().getClass() == SQLIntegrityConstraintViolationException.class) {
                return new Respuesta(false, CodigoRespuesta.ERROR_INTERNO, "No se puede eliminar la mesa porque tiene relaciones con otros registros.", "eliminarDiningTable " + ex.getMessage());
            }
            LOG.log(Level.SEVERE, "Ocurrió un error al eliminar la mesa.", ex);
            return new Respuesta(false, CodigoRespuesta.ERROR_INTERNO, "Ocurrió un error al eliminar la mesa.", "eliminarDiningTable " + ex.getMessage());
        }
    }
}
