...
public class DiningTableController {
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Mesa de comedor encontrada", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = DiningTableDTO.class))),
        @ApiResponse(responseCode = "404", description = "Mesa de comedor no encontrada", content = @Content(mediaType = MediaType.TEXT_PLAIN)),
    ...
    public Response getDiningTable(
            @Parameter(description = "ID de la mesa de comedor")
        ...
        try {
            ...
            return Response.status(CodigoRespuesta.ERROR_INTERNO.getValue())
                    .entity("Error obteniendo la mesa de comedor.").build();
        }
    }
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Mesas de comedor encontradas", content = @Content(mediaType = MediaType.APPLICATION_JSON)),
        @ApiResponse(responseCode = "404", description = "No hay mesas de comedor registradas", content = @Content(mediaType = MediaType.TEXT_PLAIN)),
        @ApiResponse(responseCode = "500", description = "Error interno", content = @Content(mediaType = MediaType.TEXT_PLAIN))
    })
    public Response getDiningTables() {
        try {
            ...
            return Response.ok(new GenericEntity<List<DiningTableDTO>>((List<DiningTableDTO>) r.getResultado("DiningTables")) {
            }).build();
        } catch (Exception ex) {
            Logger.getLogger(DiningTableController.class.getName()).log(Level.SEVERE, null, ex);
            return Response.status(CodigoRespuesta.ERROR_INTERNO.getValue())
                    .entity("Error obteniendo las mesas de comedor").build();
        }
    }
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Mesa de comedor guardada exitosamente", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = DiningTableDTO.class))),
        @ApiResponse(responseCode = "404", description = "Mesa de comedor no encontrada", content = @Content(mediaType = MediaType.TEXT_PLAIN)),
        @ApiResponse(responseCode = "500", description = "Error interno", content = @Content(mediaType = MediaType.TEXT_PLAIN))
    })
    public Response guardarDiningTable(DiningTableDTO diningTable) {
        try {
            ...
            Logger.getLogger(DiningTableController.class.getName()).log(Level.SEVERE, null, ex);
            return Response.status(CodigoRespuesta.ERROR_INTERNO.getValue())
                    .entity("Error guardando la mesa de comedor.").build();
        }
    }
    ...
    public Response eliminarDiningTable(
        ...
        try {
            ...
            Logger.getLogger(DiningTableController.class.getName()).log(Level.SEVERE, null, ex);
            return Response.status(CodigoRespuesta.ERROR_INTERNO.getValue())
                    .entity("Error eliminando la mesa de comedor.").build();
        }
    }
}
