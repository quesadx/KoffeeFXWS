...
public class AppUserController {
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Usuario Autenticado", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = AppUserDTO.class))),
    ...
    public Response validarUsuario(
            @Parameter(description = "Codigo de usuario")
            @PathParam("usuario") String usuario,
            @Parameter(description = "Clave de usuario")
        ...
        try {
            ...
            return Response.status(CodigoRespuesta.ERROR_INTERNO.getValue())
                    .entity("Error validando el usuario.").build();
        }
    }
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Renovación exitosa del token", content = @Content(mediaType = MediaType.TEXT_PLAIN)),
    ...
    public Response renovarToken() {
        try {
            ...
            if (usuarioRequest != null && !usuarioRequest.isBlank()){
                return Response.ok(JwTokenHelper.getInstance()
                .generatePrivateKey(usuarioRequest)).build();
            } else {
                return Response.status(CodigoRespuesta.ERROR_PERMISOS.getValue())
                    .entity("No se pudo renovar el token").build();
            }
        } catch (Exception ex) {
            Logger.getLogger(AppUserController.class.getName()).log(Level.SEVERE, null, ex);
            return Response.status(CodigoRespuesta.ERROR_INTERNO.getValue())
    ...
    @SecurityRequirement(name = "jwt-auth")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Usuario encontrado", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = AppUserDTO.class))),
    ...
    public Response getAppUserByUsername(
            @Parameter(description = "Username del usuario")
    ...
    @SecurityRequirement(name = "jwt-auth")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Usuarios encontrados", content = @Content(mediaType = MediaType.APPLICATION_JSON)),
        @ApiResponse(responseCode = "404", description = "No hay usuarios registrados", content = @Content(mediaType = MediaType.TEXT_PLAIN)),
        @ApiResponse(responseCode = "500", description = "Error interno", content = @Content(mediaType = MediaType.TEXT_PLAIN))
    })
    public Response getAppUsers() {
        try {
            ...
            return Response.ok(new GenericEntity<List<AppUserDTO>>((List<AppUserDTO>) r.getResultado("AppUsers")) {
            }).build();
        } catch (Exception ex) {
            Logger.getLogger(AppUserController.class.getName()).log(Level.SEVERE, null, ex);
            return Response.status(CodigoRespuesta.ERROR_INTERNO.getValue())
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Usuario guardado exitosamente", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = AppUserDTO.class))),
        @ApiResponse(responseCode = "404", description = "Usuario no encontrado", content = @Content(mediaType = MediaType.TEXT_PLAIN)),
        @ApiResponse(responseCode = "500", description = "Error interno", content = @Content(mediaType = MediaType.TEXT_PLAIN))
    })
